...
// 내 주소로 좌표를 검색합니다
geocoder.addressSearch('강서로17가길46', function(result, status) {
// 정상적으로 검색이 완료됐으면 
...
// 친구 주소로 좌표를 검색합니다
geocoder.addressSearch('까치산역', function(result, status) {
  // 정상적으로 검색이 완료됐으면 
    if (status === kakao.maps.services.Status.OK) {
      ...
      // 인포윈도우로 장소에 대한 설명을 표시합니다
      let infowindow = new kakao.maps.InfoWindow({
          content: '<div style="width:150px;text-align:center;padding:6px 0;">친구 위치</div>'
      });
      infowindow.open(map, marker);
